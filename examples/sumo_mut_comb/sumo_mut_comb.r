...
pdb.file <- "senp2_sumo4.pdb" # Parent PDB file name
...
## Generates list of individual mutations to be used in combinations ##
## res is residue number (relative to sequence of complex, hence the +228) ##
## aa is the amino acids to which the given positions (res) should be mutated ##
muts <- data.frame(res=c(21,51,63,69,72),aa=c("A","A","D","E","A"))

## Generates PQR files for all possible combination of muts given 3(max in current implementation) mutations per analog ##
pqrs <- mut.comb.c(AB,chain.names,muts,3,inputchain="b")
...
save(b.esd,b.hc,file=paste(chain.names[1],chain.names[2],"clust.dat",sep="_"))
